...
public class MessageAction extends SuperAction implements ModelDriven<Message> {
	...
	public String queryMessageList() throws Exception{
		...
		Page result = messageService.getMessageList((Integer) session.get(CommonConstant.SESSION_ID), sidx, sord, page, rows);
...
//		System.out.println("…………………………………………………MessageAction：……………………………………………………………………………");
	...
	public String countNotReadMessage()throws Exception{
		System.out.println("-------------------进入countNotReadMessage---------------------");
		Map session = ActionContext.getContext().getSession();
		long count=this.messageService.countNotReadMessage((Integer) session.get(CommonConstant.SESSION_ID));
		/** 获取response对象 */  
        HttpServletResponse response = ServletActionContext.getResponse();  
        /** 获取输出out对象 */  
        PrintWriter out = response.getWriter(); 
	...
	public String deleteMessage() throws Exception{
		System.out.println("…………………………………………………MessageAction：消息删除……………………………………………………………………………");
